...
This pack runs against a connected google_sheets account, so it needs a Pipeworx key: sign in at https://pipeworx.io/account, connect google_sheets, then call `POST https://gateway.pipeworx.io/v1/tools/sheets_read` with `Authorization: Bearer <your Pipeworx key>`. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/sheets_read`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
